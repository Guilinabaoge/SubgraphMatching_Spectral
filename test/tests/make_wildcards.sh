    ...
    do 
        ...
        do
            ...
            do
                ...
                do
                query_path="../reallife_dataset/${dataset}/query_graph/query_${prop_val}_${size}_${query_number}.graph"
                python3 generate_wildcard_query.py -s "$size" -prop "$prop_val" -d $dataset -q "$query_path" -p "$p" -n "$query_number"
                echo "$size" -prop $prop_val -d $dataset -q $query_path -p $p -n $query_number
                done
            done
        done 
    done
